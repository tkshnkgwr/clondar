...
### 📅 日本の祝日（法改正）への対応手順
日本の祝日が追加・変更された場合、以下の手順で祝日判定ロジックを更新します。
1. [ui/index.html](./ui/index.html) を開きます。
2. `HolidayLogic` オブジェクト内の `getHolidays` メソッドを探します。
3. 法改正に合わせて、該当する日付と祝日名を `holidays` オブジェクトに追加または条件変更します。
   * 例：`holidays["${year}-08-11"] = "山の日";` のような固定祝日や、ハッピーマンデー・天文計算ロジックを修正します。
4. 修正後、`cargo tauri dev` で動作確認を行い、[TEST_REPORT.md](./TEST_REPORT.md) を更新します。

### 📦 リリース・バージョンアップ手順
新しいバージョンをビルド・リリースする際は、以下のチェックリストを順に実行します。

1. **バージョン表記の更新 (3箇所)**:
   * [Cargo.toml](./Cargo.toml) の `version` フィールド
   * `src-tauri/tauri.conf.json` の `version` フィールド（`package > version`）
   * [ui/index.html](./ui/index.html) の `WindowFrame` 内のバージョン表示（例: `v1.2.0`）
2. **アプリアイコンの更新 (変更時のみ)**:
   * 新しい 512x512px 以上の正方形画像を用意し、`npx tauri icon /path/to/icon.png` を実行。
   * アイコン変更をビルドに反映させるため、`cargo clean` を実行。
3. **ビルドの実行**:
   * `cargo tauri build` を実行し、`src-tauri/target/release/bundle/` 下にインストーラーが生成されることを確認。
4. **CHANGELOG の更新**:
