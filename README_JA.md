...
4. 動作に問題がないことを確認し、[TEST_REPORT.md](./docs/ja/TEST_REPORT.md) などを更新します。
...
1. **バージョン表記の一括更新**:
   * PowerShell 7 (`pwsh`) を開き、以下のコマンドを実行します：
     ...
     ```
     これにより、`Cargo.toml`、`tauri.conf.json`、`docs/ja/SPECIFICATION.md`、`docs/ja/TEST_REPORT.md` 内のバージョン情報が自動的に一括置換されます。
2. **アプリアイコンの更新 (変更時のみ)**:
   * 新しい 512x512px 以上の正方形画像を用意し、`npx tauri icon /path/to/icon.png` を実行。
   * アイコン変更をビルドに反映させるため、`cargo clean` を実行。
3. **CHANGELOG の更新**:
   * [CHANGELOG.md](./docs/ja/CHANGELOG.md) に新規バージョンと変更点を追記。
4. **自動ビルド＆デプロイのトリガー**:
