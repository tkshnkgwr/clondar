...
## 5. コーディング規約とドキュメンテーションコメント

本プロジェクトでは、コードの可読性と保守性を担保するため、以下のコメント・ドキュメント規約を設けています。

### ① ドキュメントコメント（RustDoc / JSDoc）の作成と同期
コードの追加や仕様変更時には、必ず対応するドキュメントコメント（RustDoc / JSDoc）を作成または更新してください。
* **Rust**: クレート、モジュール、構造体、列挙型、関数、メソッドなどに対して、`///` または `//!` を用いて RustDoc コメントを記述します。
* **JavaScript / React**: 各コンポーネント、カスタムフック、ユーティリティ関数などに対して、`/** ... */` を用いて JSDoc コメントを記述します。引数には `@param`、戻り値には `@returns` を適切に設定します。

### ② コメントの言語設定
コード内のコメントおよびドキュメントコメントは、原則として**日本語で統一**します。英語のコメントが残っている場合は、本プロジェクト開発時および仕様変更のタイミングで日本語にローカライズ（日本語化）してください。
